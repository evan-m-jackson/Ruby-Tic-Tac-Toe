...
describe 'Play' do
  ...
  describe '.run' do
    ...
    it 'prints a welcome message' do
      ...
      @play = Play.new(@test_board, @cpu, @player, @end_of_game, output: @test_ui)
      @play.run
    ...
    it 'prints the board' do
      ...
      @test_player = TestPlayerChoice.new(input: ['2'], output: @test_ui)
      @play = Play.new(@test_board, @cpu, @test_player, @end_of_game, output: @test_ui)
      @play.run
    ...
    it 'Player chooses an invalid move and an error message is printed' do
      @test_board.board = %w[X 2 X 4 5 6 7 8 9]
      @test_player = TestPlayerChoice.new(input: %w[a 2], output: @test_ui)
      @play = Play.new(@test_board, @cpu, @test_player, @end_of_game, output: @test_ui)
      @play.run
    ...
    it 'Player chooses a spot that is already taken and an error message is printed' do
      @test_board.board = %w[X 2 X 4 5 6 7 8 9]
      @test_player = TestPlayerChoice.new(input: %w[1 2], output: @test_ui)
      @play = Play.new(@test_board, @cpu, @test_player, @end_of_game, output: @test_ui)
      @play.run
    ...
    it 'The game ends in a draw and the tie message is printed' do
      ...
      @test_player = TestPlayerChoice.new(input: ['7'], output: @test_ui)
      @play = Play.new(@test_board, @cpu, @test_player, @end_of_game, output: @test_ui)
      @play.run
